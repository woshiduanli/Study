...
public class FileTool
{
    ...
    /// <param name="searchPattern">搜索后缀名模式</param>
    /// <param name="reserveEmptyDirectory">是否保留空文件夹</param>
    public static void CopyDirectory(string sourcePath, string destinationPath, bool overwrite = true, string searchPattern = "*", SearchOption option = SearchOption.AllDirectories, bool reserveEmptyDirectory = false)
    {
        ...
        if(!reserveEmptyDirectory)
        {
            string[] files = Directory.GetFiles(sourcePath, searchPattern, option);
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                string dest = destinationPath + "/" + str;
        ...
        else
        {
            ...
            foreach (FileSystemInfo fsi in info.GetFileSystemInfos())
            {
                ...
                {
                    if (searchPattern.Contains(fsi.Extension) || searchPattern == "*")
                    {
                ...
                else                                    //如果是文件夹，新建文件夹，递归
                {
                    ...
                    {
                        CopyDirectory(fsi.FullName, destName, overwrite, searchPattern);
                    }
                }
            }
        }
    ...
    /// <param name="sourcePath">待复制的文件夹路径</param>
    /// <param name="destinationPath">目标路径</param>
    public static void SafeCopyDirectory(string sourcePath, string destinationPath, bool overwrite = true, string searchPattern = "*")
    {
        DirectoryInfo info = new DirectoryInfo(sourcePath);
        Directory.CreateDirectory(destinationPath);
        ...
        foreach (FileSystemInfo fsi in info.GetFileSystemInfos())
        {
            ...
            {
                Directory.CreateDirectory(destName);
                SafeCopyDirectory(fsi.FullName, destName, overwrite, searchPattern);
            }
        }
    }
